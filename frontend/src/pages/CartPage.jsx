  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 grid md:grid-cols-[1fr_320px] gap-8">
      <div className="space-y-4">
        {items.map((item) => (
          <div key={item.variantId} className="flex gap-4 bg-white rounded-2xl p-4 shadow-sm">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-2">
                ...
                <button onClick={() => removeItem(item.variantId)} className="text-xs text-blush-dark">Remover</button>
              </div>
            </div>
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm h-fit space-y-3">
        ...
        <div className="flex gap-2 pt-2">
          ...
          <button onClick={applyCoupon} className="text-sm bg-neutral-800 text-white rounded-lg px-3">Aplicar</button>
        </div>
        ...
        <button onClick={() => navigate('/checkout', { state: { couponCode: couponResult ? coupon : null } })}
          className="w-full bg-aqua text-white rounded-full py-3 font-medium hover:bg-aqua-dark">
          Finalizar Compra
        </button>
      </div>
    </div>
